...
**小号 IV（3776580862）在群 985100027 里像真实群友一样互动**：会窥屏、会潜水、会插话、会接梗，不暴露 AI 身份（不主动声明、被问不撒谎）。
...
2. 过滤：只处理 `message_type=group` 且 `group_id=985100027`；忽略 `user_id=3776580862`（自己）、忽略匿名/其他 bot；Four 私聊（`user_id=2337529577`）走 master 命令通道
...
| 触发级别 | 条件 | 动作 |
|---|---|---|
| 必回 | 被 @（CQ:at qq=3776580862）、被引用回复 | 立即进入生成流程（不受冷却/暂停限制） |
...
- v1.6 2026-09-21 换号迁移：旧号 2721212523 被 QQ 锁定 → 新号 **3776580862**（沿用 IV 人设）。新号 WebUI 扫码登录 + `webui.json autoLoginAccount` 指向新号；**新账号的 `onebot11_<新QQ>.json` 是全新默认配置，必须从旧账号配置复制 network 段（HTTP :3000 + webhook :3210），否则 OneBot 永远不起**；`config.json botQQ` 与文档同步更新
 - v1.7 2026-09-21 拟人化修正：对比群消息记录发现 bot 说话太"成品段子"（每条对仗抖包袱、永远两连发、逢聊必到、纯点评视角）。persona.md 增「碎片化」（允许单字/单词回复、连发随机 1~3 条）与「少点评」规则；顺带修 webhook 响应 `'ok'`→`'{}'`（消除 NapCat JSON5 解析噪音，每条消息一条 ERROR）
...
- v1.8 2026-09-21 失联自愈（同日两次 NapCat/QQ 被踢失联后 Four 拍板）：①`config\napcat_3776580862.json` 开 `fileLog`——掉线原因不再靠考古；②controller.cjs 新增**存活监控**——每 60s 探测 `get_login_info`，连续 3 次失败确认失联 → **Win11 Toast 通知**（Four 否决响铃方案）+ 自动重启 NapCat 层（杀全部 QQ 后带 botQQ 参数重拉 launcher，等 90s）；30 分钟内最多自动重启 3 次（防抖动死循环），超限或从未上线（扫码中）只报警不动手；`stop` 停监控，`status` 显示监控状态。事故归因：两次掉线均为 QQ 客户端侧被杀/被踢（反作弊或风控），bot/LLM 链路无恙；疑似诱因含同机双开主号 QQ，手册已加注意事项；③**登录引导程序内闭环**（Four 要求）——start 拉起 NapCat 即打印管理 URL；60s 未登录/监控发现未登录/自动重启 3 次失败时自动打开浏览器到 WebUI 登录页（二维码所在），并从 NapCat fileLog 提取 `ptlogin2` 二维码直链打印到终端；④**思考强度 max**（Four 拍板）——config `llm.reasoningEffort`（low/medium/high/max，实测 max 思考 token ≈ 默认 2.5 倍，已验证 API 接受该枚举），仅作用于主模型，failover 切 fallback 时自动摘除该参数；bot.cjs 需重启一次识别此参数；⑤**多模型注册表 + `/模型` 切换命令**——`llm.models` 注册表支持 Anthropic Messages 格式模型，首个实例 `kimi-k3`（Kimi K3-256K，`https://api.kimi.com/coding/v1/messages`，key 存 secrets.json kimiCodeKey，思考 low，订阅制计价 0）；私聊 `/模型` 查看/`/模型 <名>` 模糊匹配切换，选择持久化在 ctx.modelOverride 重启后恢复；自定义模型不做自动 failover（失败重试 3 次后保持原模型）；Anthropic 路径超时放宽 90s
