...
- v1.8 2026-09-21 失联自愈（同日两次 NapCat/QQ 被踢失联后 Four 拍板）：①`config\napcat_3776580862.json` 开 `fileLog`——掉线原因不再靠考古；②controller.cjs 新增**存活监控**——每 60s 探测 `get_login_info`，连续 3 次失败确认失联 → 响铃 + 自动重启 NapCat 层（杀全部 QQ 后带 botQQ 参数重拉 launcher，等 90s）；30 分钟内最多自动重启 3 次（防抖动死循环），超限或从未上线（扫码中）只报警不动手；`stop` 停监控，`status` 显示监控状态。事故归因：两次掉线均为 QQ 客户端侧被杀/被踢（反作弊或风控），bot/LLM 链路无恙；疑似诱因含同机双开主号 QQ，手册已加注意事项
